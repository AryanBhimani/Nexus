...
import com.ABMTechnoConsultants.nexus.MainActivity
...
class PhoneAuthViewModel @Inject constructor(
    ...
    fun sendVerificationCode(phoneNumber: String, activity: Activity){
        ...
        val option = object : PhoneAuthProvider.OnVerificationStateChangedCallbacks(){
            override fun onCodeSent(id: String, token: PhoneAuthProvider.ForceResendingToken) {
                ...
                Log.d("PhoneAuth", "onCodeSent triggerd. verification Id: $id")
        ...
        firebaseAuth.signInWithCredential(credential)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    val phoneNumber = PhoneAuthUser(
                        userId = user?.uid ?: "",
                        phoneNumber = user?.phoneNumber ?: "",
                    )
                    marUserAsSignedIn(context)
                    _authState.value = AuthState.Success(phoneNumber)
    ...
    fun marUserAsSignedIn(context: Context){
    ...
    fun verifyCode(otp:String, context: Context){
        ...
        if (currentAuthState !is AuthState.CodeSent || currentAuthState.verificationId.isEmpty()){
            Log.e("PhoneAuth", "Attempted to verify OTP without valid verification ID")
    ...
    fun saveUserProfile(
        ...
        val encodeImage = profileImage?.let {converBitmapToBase64(it) }
    ...
    private fun converBitmapToBase64(bitmap: Bitmap): String{
